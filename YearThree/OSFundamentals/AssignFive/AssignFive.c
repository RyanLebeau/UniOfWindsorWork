...
int main(int argc, char* argv[]){
	...
	int oneP[2], twoP[2], threeP[2], fourP[2], fiveP[2], parentP[2], parentPass[2], pids[5], count=10, ppid = getpid(), status;
	char* msgRec;
	char* msgRecC;
	char* msgRecP;
	pipe(oneP); pipe(twoP); pipe(threeP); pipe(fourP); pipe(fiveP); pipe(parentP); pipe(parentPass);
		...
		while(1){
			...
			if(count==1){
				...
				if(!strcmp(msgRec, msg)){
					...
					close(twoP[1]);
					//child passing token to parent pipe
					close(parentPass[0]);
					write(parentPass[1], &msg, MSGSIZE);
					close(parentPass[1]);
					exit(count);
				}
			}
			//child two
			if(count==2){
				//reading from the parent pipe
				close(parentP[1]);
				read(parentP[0], &msgRec, sizeof(msgRec));
				close(parentP[0]);
				//reading from the child pipe
				close(twoP[1]);
				read(twoP[0], &msgRecC, sizeof(msgRecC));
				close(twoP[0]);
				if(!strcmp(msgRec, msg) && !strcmp(msgRecC, "child1_done")){
					//inside child process
					printf("I am kid#:%d and my id is %d\n", count, getpid());
					//using the correct fifth of the sorted array per child
					int persis[25];
			...
			if(count==3){
				...
				//reading from the child pipe
				close(threeP[1]);
				read(threeP[0], &msgRecC, sizeof(msgRecC));
				close(threeP[0]);
				if(!strcmp(msgRec, msg) && !strcmp(msgRecC, "child2_done")){
					//inside child process
					printf("I am kid#:%d and my id is %d\n", count, getpid());
					//using the correct fifth of the sorted array per child
					int persis[25];
			...
			if(count==4){
				...
				//reading from the child pipe
				close(fourP[1]);
				read(fourP[0], &msgRecC, sizeof(msgRecC));
				close(fourP[0]);
				if(!strcmp(msgRec, msg) && !strcmp(msgRecC, "child3_done")){
					//inside child process
					printf("I am kid#:%d and my id is %d\n", count, getpid());
					//using the correct fifth of the sorted array per child
					int persis[25];
			...
			if(count==5){
				...
				//reading from the child pipe
				close(fiveP[1]);
				read(fiveP[0], &msgRecC, sizeof(msgRecC));
				close(fiveP[0]);
				if(!strcmp(msgRec, msg) && !strcmp(msgRecC, "child4_done")){
					//inside child process
					printf("I am kid#:%d and my id is %d\n", count, getpid());
					//using the correct fifth of the sorted array per child
					int persis[25];
	...
	while(1){
		...
		//reading from the parent pipe
		close(parentPass[1]);
		read(parentPass[0], &msgRecP, sizeof(msgRecP));
		close(parentPass[0]);
		if(!strcmp(msgRecP,"child1_done")){
			//passing GO_AHEAD token to parent pipe
			msg="GO_AHEAD";
			close(parentP[0]);
			write(parentP[1], &msg, MSGSIZE);
		}
		if(!strcmp(msgRecP,"child2_done")){
			//passing GO_AHEAD token to parent pipe
			msg="GO_AHEAD";
			close(parentP[0]);
			write(parentP[1], &msg, MSGSIZE);
		}
		if(!strcmp(msgRecP,"child3_done")){
			//passing GO_AHEAD token to parent pipe
			msg="GO_AHEAD";
			close(parentP[0]);
			write(parentP[1], &msg, MSGSIZE);
		}
		if(!strcmp(msgRecP,"child4_done")){
			//passing GO_AHEAD token to parent pipe
			msg="GO_AHEAD";
			close(parentP[0]);
			write(parentP[1], &msg, MSGSIZE);
